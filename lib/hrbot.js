...
HRBot.prototype._replyWithMessageFromList = function (originalMessage, reply) {
    var self = this;
    var user = self._getUserById(originalMessage.user);
    var channel = self._getChannelById(originalMessage.channel);

    // self.getSavedMessages(user.name);
    ...
    self.saveMessage(user.name, reply);
}
...
HRBot.prototype._onMessage = function (message) {
  ...
  if (this._isChatMessage(message) &&
      ...
      // Interesting feature to consider from rench by saving a json of triggers.
      ...
      for (var i = 0; i < triggers.length; i++) {
          ...
          if (incomingMessage.indexOf(triggerKey)> -1) {
            ...
            this._replyWithMessageFromList(message, reply)
          }
        }
